...
<main>
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<input type="button" value="ID CHECK" id="btnIDCheck">
						</td>
					</tr>
					<tr>
	...
	<script>
		...
		$("#btnIDCheck").on("click",function(){
			...
			if(userId.length>=4) {
				$.ajax({
					url:"MemberIdCheck.do?id="+userId
					//GSON
				})
				.done(function(data){
					const result = Number(data.trim());
					if(result>0) {
						//password 암호화...
						...
						const use = confirm("사용할 수 있는 아이디 입니다. 사용하시겠어요");
						if(use) {
